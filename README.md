...
El MVP está **completo y funcional end-to-end**: registro, login, logout, persistencia de sesión, protección de rutas, listado de artículos, detalle con registro de visualización, publicación con carga de documento+imagen a Supabase Storage, comentarios y "me gusta" en tiempo real, y un asistente conversacional (RAG) que responde solo con el conocimiento publicado en la plataforma, citando fuentes. Toda la información proviene de Supabase (Postgres + Auth + Storage + pgvector) con RLS. La arquitectura sigue el patrón Componentes → Hooks → Services → Supabase, ahora organizada como monorepo (Turborepo + npm workspaces: `apps/web`, `apps/mcp`, `packages/*` — ver [ARCHITECTURE.md](ARCHITECTURE.md)).

Sobre la app base (sistema de diseño, layouts, navegación, componentes reutilizables, services, hooks y pantallas `(auth)`/`(dashboard)`) se construyeron, en etapas sucesivas: el sistema RAG completo (embeddings, búsqueda vectorial, construcción de contexto, chat con streaming), un servidor MCP (`apps/mcp`) que expone el conocimiento de ReadHub como tools/resources/prompts reutilizando los mismos `packages/*`, la migración a monorepo, y una suite de testing (71 pruebas unitarias con Vitest + E2E con Playwright cubriendo autenticación, publicación, likes, comentarios y el asistente) automatizada en CI/CD con GitHub Actions (ver la sección [CI/CD](#cicd)). El repositorio está publicado en GitHub con el pipeline corriendo en verde en cada push.
