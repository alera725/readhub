...
**Sesión 5 — Monorepo y servidor MCP:** la app (antes un único proyecto Next.js) se migró a un monorepo con Turborepo + npm workspaces. El código de dominio se extrajo a `packages/*` (`types`, `config`, `shared`, `ai`, `database`, `rag`) sin cambiar su comportamiento, y `apps/web` pasó a consumirlos como dependencias del workspace (`transpilePackages`, ya que exportan TypeScript fuente sin build propio). Se agregó `apps/mcp`, un segundo consumidor independiente de esos mismos packages: un servidor MCP (stdio) que expone el conocimiento de ReadHub como 11 tools (5 de consulta + 6 de análisis), 7 resources y 5 prompts, sin duplicar lógica de negocio ni importar nada de `apps/web`.

**Sesión 6 — Testing, CI/CD y publicación:** se agregó una suite de pruebas unitarias con Vitest (71 pruebas: `packages/rag`, `packages/database`, `packages/shared`, hooks/utils de `apps/web`) y E2E con Playwright (`apps/web/e2e`, patrón Page Object Model) cubriendo autenticación, el flujo de un autor (publicar → dar like → comentar sobre su propio artículo, autocontenido con datos únicos por corrida) y el asistente RAG (una pregunta real contra el pipeline completo, con aserción de que la respuesta llega con fuentes). Se configuró `.github/workflows/ci.yml` (jobs `quality` y `e2e`, artefactos de Playwright solo ante fallos) y se publicó el repositorio en GitHub (`github.com/alera725/readhub`, cuenta personal). Un fallo intermitente del job `e2e` en el primer push (`Failed to execute 'fetch' on 'Window': Invalid value`) se investigó a fondo — se descartaron credenciales desactualizadas (verificadas contra el proyecto real vía el MCP de Supabase) y una carrera de compilación de Turbopack (no reprodujo en tres réplicas fieles del runner de CI) — hasta encontrar la causa real: el secret de GitHub se había cargado corrupto por un copy-paste desde el chat; recargarlo directo desde `.env.local` lo resolvió.

Una revisión técnica posterior, con criterio de Tech Lead (arquitectura, SOLID, Clean Code, cobertura de pruebas y pipeline), aprobó el resultado con observaciones y detectó dos brechas concretas, ambas ya corregidas: la cobertura E2E se limitaba al login (no ejercitaba publicar/comentar/dar like/el asistente, ver arriba), y `extractArticleContent` (dentro de `embedding.service.ts`, sección 10.2) solo leía documentos `.txt` — PDF y DOCX, ya aceptados por el formulario de carga, se indexaban sin su cuerpo (solo título+resumen), degradando en silencio la calidad de las respuestas del asistente para la mayoría de los documentos reales. Se agregó extracción real para los tres formatos (`mammoth` para DOCX, `unpdf` para PDF), cubierta con pruebas unitarias que mockean ambos parsers en el límite.
